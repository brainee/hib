    ...
    var AppUtility = {
        ...
        openApp: function (hasAppFunc, noAppFunc, appUrl) {
            ...
            if (appData && appData != '') {
                if (appData.hasApp) {
                    if (typeof hasAppFunc == 'function') {
                        if (hasAppFunc()) { if (appUrl && appUrl.length > 0) { window.location = appUrl; } }
                    } else {
                        if (appUrl && appUrl.length > 0) { window.location = appUrl; }
                    }
                } else {
            ...
            if (isChrome) {
                ...
                window.location = "intent://m.ctrip.com/#Intent;scheme=ctrip;package=携程旅行;end";
                /*if (u.indexOf("nexus", 0) != -1) {
...
adOptions.createHtml = function () {
    ...
    var isSpecAllianceid = 0;
    if (allianceid && allianceid.length > 0 && sid && sid.length > 0) {
        sCss = 'display:none;';
        //特殊的allianceid显示浮层        
        var lstSpecAllianceid = ['5942','1127','7225','3588'];
        for (var i = 0, len = lstSpecAllianceid.length; i < len; i++) {
            if (lstSpecAllianceid[i] == allianceid) { 
                isSpecAllianceid = 1; 
                break; 
            } 
        }
        if (isSpecAllianceid) { sCss = ''; }
    }
    if ((unionInfo || unionCookie) && !isSpecAllianceid) { sCss = 'display:none;'; }
    if (s && s.sid && +s.sid > 0) {
    ...
    if (sCss.length > 0) {
        ...
        if ($('.f_list')) { $('.f_list').removeClass('pb85'); }
...
adOptions.setAppUrl = function () {
    ...
    if (pageId && +pageId > 0) {
            ...
            if (searchInfo) {
                ...
                if (+pageId == 212092) {
                    ...
                    if (searchInfo.data) {
                        ...
                        if (searchInfo.data.Qparams && searchInfo.data.Qparams.length > 0) {
                            for (var s in searchInfo.data.Qparams) {
                                var qparams = searchInfo.data.Qparams[s];
                                if (qparams && qparams.itpe) {
                                    //品牌ID
                                    if (+qparams.itpe == 3) {
                                        c5 = qparams.ival || ''; //品牌ID (可选)
                                        break;
                                    }
                                }
                            }
        ...
        if (+pageId == 212042) {
            ...
            if (searchInfo && searchInfo.data && searchInfo.data.fNo && searchInfo.data.fNo.length > 0 && searchInfo.data.dPort && searchInfo.data.dPort.length > 0 && searchInfo.data.aPort && searchInfo.data.aPort.length > 0) {
                c1 = searchInfo.data.fdate;
                c2 = searchInfo.data.fNo;
                c3 = searchInfo.data.dPort;
                c4 = searchInfo.data.aPort;
                bizName += '?c1=' + c1 + "&c2=" + c2 + "&c3=" + c3 + "&c4=" + c4;
            }
            //c1 查询日期（yyyymmdd）
            //c2 航班号 （必需）
            //c3 出发机场三字码（必需）
        ...
        if (+pageId == 212003 || +pageId == 212004 || +pageId == 212009 || +pageId == 214019 || +pageId == 214209) {
            ...
            if (window.localStorage) {
                searchInfo = window.localStorage.getItem("S_FLIGHT_AirTicket") || window.localStorage.getItem("FLIGHT_SEARCH");
                subInfo = window.localStorage.getItem("S_FLIGHT_SUBJOIN") || window.localStorage.getItem("FLIGHT_SEARCH_SUBJOIN");
            }
            searchInfo = searchInfo ? JSON.parse(searchInfo) : null;
            if (searchInfo && searchInfo.value) {
                c1 = searchInfo.value.tripType; //单程/往返(1/2)（必需）
                if (searchInfo.value._items && searchInfo.value._items.length > 0) {
                    c2 = searchInfo.value._items[0].dCtyId; //出发城市id （必需）
                    c3 = searchInfo.value._items[0].aCtyId || searchInfo.value.segments[0].aCtyId; //到达城市id（必需）
                    if (searchInfo.value._items[0].date)
                        c4 = searchInfo.value._items[0].date.replace(_reg2, ''); //出发时间（yyyymmdd）（必需）
                }
                else {
                    if (searchInfo.value.segments && searchInfo.value.segments.length > 0) {
                        c2 = searchInfo.value.segments[0].dCtyId;
                        c3 = searchInfo.value.segments[0].aCtyId; //到达城市id（必需）
                        c4 = (searchInfo.value.segments[0].dDate ? searchInfo.value.segments[0].dDate.replace(_reg2, '') : ''); //出发时间（yyyymmdd）（必需）
                    }
                }
                if (c1 && +c1 > 1) {
                    if (searchInfo.value._items && searchInfo.value._items.length > 1 && searchInfo.value._items[1].date)
                        c5 = searchInfo.value._items[1].date.replace(_reg2, ''); //出发时间（yyyymmdd）（必需）
                    else {
                        c5 = (searchInfo.value.segments[1].dDate ? searchInfo.value.segments[1].dDate.replace(_reg2, '') : ''); //出发时间（yyyymmdd）（必需）
                    }
                }
                if (+pageId == 214019 || +pageId == 214209) {
                    subInfo = null;
                    if (window.localStorage) {
                        subInfo = window.localStorage.getItem("S_FLIGHT_INTLAirTicket") || window.localStorage.getItem("FLTINTL_SEARCH");
                    }
                }
                c4 = c4 ? c4.replace(_reg, '') : "";
                c5 = c5 ? c5.replace(_reg, '') : "";
                    ...
                    if (+pageId == 214019 || +pageId == 214209) {
                        ...
                        */
                        ...
                        if (+subInfo.value.sortRule == 2) {
                            //1:起飞时间升序 2:起飞时间降序 
                            if (+subInfo.value.sortType == 2) { c8 = "1"; }
                            if (+subInfo.value.sortType == 1) { c8 = "2"; }
                        }
                        if (+subInfo.value.sortRule == 1) {
                            //3:价格升序 4：价格降序 5:耗时升序6:耗时降序
                            if (+subInfo.value.sortType == 2) { c8 = "3"; }
                            if (+subInfo.value.sortType == 1) { c8 = "4"; }
                        }
                        if (+pageId == 214209) {
                            c7 = c8 = c9 = '';
                        }
...
adOptions.onShow = function () {
    ...
    this.root.find('#app_link').off('click').on('click', function (e) {
        e.preventDefault();
        // 修改this指向错误  slh
        var url = $(this).attr('href'), appUrl = scope.setAppUrl(), pageId = $('#page_id').val(),u = navigator.userAgent ? navigator.userAgent.toLocaleLowerCase() : '';
...
adOptions.saveExpire = function (isClose) {
    ...
    if (isClose) { data.isClose = isClose; }
...
adOptions.checkAutoDownload = function () {
    //调整自动下载需求：必须传入
    var self = this, sourceid = this.getUrlParam('sourceid'),
    isopenapp = this.getUrlParam('openapp'),
    isodownapp = this.getUrlParam('downapp');

    if (!sourceid || sourceid.length <= 0 || +sourceid <= 0) return;
    var isdown_iosapp = 0, isdown_androidapp = 0, isopen_androidapp = 0, isopen_iosapp = 0;
    var u = navigator.userAgent ? navigator.userAgent.toLocaleLowerCase() : '';
    //判断设备类型
    ...
    if (isopen_androidapp || isopen_iosapp) {
        appUrl = adOptions.setAppUrl(); //生产app协议url
    }
    var s = adOptions._get("SALES_OBJECT"); //获取渠道信息
    var isDown = adOptions.isAutoDown(sourceid);
    if (isMac) {
        if (appUrl && appUrl.length > 0) {
            window.location = appUrl;
        }
        /************判断是否已经强制下载，若已强制下载则不再执行自动下载************/
        if (isDown || !isdown_iosapp || +isdown_iosapp != 1) {
        ...
        AppUtility.openApp(function () {
            ...
            /**************判断用户网络环境，若不是wifi环境，则不自动下载 android 360浏览器除外(2014-03-25需求调整)****************/
            var is360 = (u.indexOf("360browser", 0) != -1) || (navigator.userAgent.indexOf("360", 0) != -1) ? 1 : 0; //是否是360浏览器
            if (isAndroid && !is360) {
                if (navigator.connection) {
                    if (navigator.connection.type != navigator.connection.WIFI) {
                        return;
...
adOptions.create = function () {
    if (this.isCreate || this.isExpire() || (this.status && this.status == this.STATE_ONCREATE)) {
          if ($('footer')) { $('footer').removeClass('pb85'); }
        if ($('#panel-box')) { $('#panel-box').removeClass('pb85'); }
        if ($('.f_list')) { $('.f_list').removeClass('pb85'); }
        if ($('div[data-role="footer"]')) { $('div[data-role="footer"]').removeClass('pb85'); }
    ...
    if (!window.___checkAutoDownloadRun) {
        window.___checkAutoDownloadRun = true;
        var _this = this;
        setTimeout(function () {
            _this.checkAutoDownload();
        }, 2500);
    }
};
...
var p = window.location.pathname ? window.location.pathname.toLowerCase() : '';
if (p.indexOf('webapp') > -1 || window.localStorage.getItem('isInApp')) {
