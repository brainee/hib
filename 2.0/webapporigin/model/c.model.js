...
*/
define(['libs', 'cBase', 'cStore', 'cUtility', 'CommonStore', 'cAbstractModel', 'cStorage', 'cUtilityPath'],
  function (libs, cBase, AbstractStore, cUtility, CommonStore, baseModel, cStorage, pathUtil) {
          ...
          */
          ...
          auth = "", success = false;
              if (fromSOA) {
                  ...
                  //SOA2.0的成功判断增加枚举类型
                  success = (head && (head.Ack === 'Success' || head.Ack == '0'));
              } else {
                  auth = head.auth;
      ...
      AbstractModel.baseurlold = function (protocol) {
          ...
          if (cUtility.isInApp()) {
              if (cUtility.isPreProduction() == '1') { // 定义堡垒环境
                  ...
                  if (protocol == "https") {
                      domain = 'restful.m.uat.nt.qa.ctripcorp.com';
                  } else {
              ...
              if (AbstractModel.isHttps(protocol)) {
                  domain = 'restful.m.uat.qa.nt.ctripcorp.com';
              } else {
                  domain = 'm.uat.qa.nt.ctripcorp.com';
              }
      ...
      AbstractModel.baseurl = function (protocol) {
          ...
          var domain = 'm.ctrip.com';
          var path = 'restapi';
          if (Lizard.restfullApiHttps && protocol == "https") {
              domain = pathUtil.parseUrl(Lizard.restfullApiHttps).hostname;
              return {
                  'domain': domain,
                  'path': path
              }
          }
          else if (Lizard.restfullApi && protocol == "http") {
              domain = pathUtil.parseUrl(Lizard.restfullApi).hostname;
              return {
                  'domain': domain,
                  'path': path
              }
          } else {
              var ret = AbstractModel.baseurlold(protocol);
              return ret;
          }
