define(['CommonStore', 'cMessageCenter', 'cUILoading', 'cUIWarning404', 'cUIHeader', 'cBase', 'cModel', 'memStore', 'cUtilityPath', 'cUtilityUtil', 'cUtilityStateHistory', 'cUtilityPerformance', 'cUtilityCacheView', 'cUIAnimation', 'cUtilityFlip', 'cPageParser'],
  function (CommonStore, MessageCenter, Loading, Warning404, cUIHeader, Base, cModel, mStore, path, utils, History, Performance, CacheViews, animation) {
          ...
          start: function start() {
              ...
              */
              ...
              if (!Lizard.isHybrid) {
                  ...
                  this.loadView(
                    fistPageURL.substring(pathBeginIndex),
                    fistPageHtml,
                    { 
                  ...
                  **/
                  var defaultView = chistory.getHash();
                  if (!defaultView) {
                      if (Lizard.localRoute.config.defaultView) {
                        defaultView = Lizard.localRoute.config.defaultView;
                      } else {
                        defaultView = _.first(_.keys(Lizard.localRoute.config)).replace(/(\{(.+?)\})/g, function (a, b) {
                          return "";
                        });                       
                      }
          ...
          _handleLink: function _handleLink() {
              ...
              $('body').on('click', $.proxy(function (e) {
                  ...
                  if (needhandle) {
                      ...
                      if (/*el.attr('target') || */(el.attr('lizard-catch') == 'off') || (href && utils.isExternalLink(href))) {
                          return true;
                      }
      ...
      this));
          ...
          _renderCallback: function (ret, isServer, opts, uuidDomready, uuidOnload, url, text, pushState) {
              ...
              */
              ...
              var $el = isServer ? $(this.viewport[0].firstChild) : $(ret.viewport);
              if (!isServer) {
                  ...
                  /*
                  添加对popstate、hashchange过来的loadview，opts.id存在代表是从popstate、hashchange过来的
                  */
                  if (opts.stateid) {
              ...
              var state = {
                  id: isServer ? $el.attr("id") : (opts.stateid || ret.id), //优先考虑opts中的id
          ...
          parsePage: function parsePage(url, html, callback, callbackarg) {
              ...
              this.getData(pageConfig, function (datas) {
                  ...
                  if (renderObj.config.url_schema) {
                      ...
                      _.each(renderObj.config.url_schema, function (urlSchema) {
                          var parseRet = Lizard.schema2re(urlSchema, url);
                          if (parseRet.reStr && !Lizard.urlSchemaMap[parseRet.reStr]) {
                              Lizard.urlSchemaMap[parseRet.reStr] = { html: html, urlMatchIndex: parseRet.index };
                          }
                      });
                  }
          ...
          getData: function getData(pageConfig, callback) {
              ...
              Lizard.ajaxDatas = {}; Lizard.errorCalled = false;
          ...
          _onSucess: function (data, datas, index, models, callback, ignoreError) {
              ...
              if (_.every(models, function (model) {
                  return model.done;
              })) {
          ...
          _processModels: function (models, callback, datas, ignoreError) {
              ...
              if (_.some(models, function (model) {
                  ...
                  var errorData = {
                      ...
                      headData: { title: '网络不给力', events: { returnHandler: function () {
                          waring404.hide();
                  ...
                  if (Lizard.errorBack)
                      errorData = _.extend(errorData, Lizard.errorBack(datas));
                  var waring404 = new Warning404();
                  waring404.show();
                  waring404.options(errorData);
                  if (errorData.tel) waring404.tel = errorData.tel;
                  waring404.callback = function () { errorData.callback.call(this), waring404.hide() };
                  var hDom = $('#headerview');
                  if (errorData.headData) {
                      var headerview = new cUIHeader({ 'root': hDom });
              ...
              _.each(models, function (model, index) {
                  ...
                  if (!model.depends || _.every(model.depends, function (depend) {
                      ...
                      if (!emodel) {
                          emodel = new Base.Class(cModel, { __propertys__: function () {
                      ...
                      if (model.suspend && eval('(' + model.suspend + ')()')) {
                          _this._onSucess({}, datas, index, models, callback);
                      }
                      else {
          ...
          controller: function controller(params) {
              ...
              this.loadController(controller, $.proxy(function (View) {
                  if (View) {
                      ...
                      this.curView['onmessage'] = this.curView['onmessage'] || function (data) { Lizard.log('onmessage', data); }
      ...
      this));
          ...
          loadController: function loadController(controller, callback) {
              ...
              */
              ...
              requirejs([controller/*+"?t="+(new Date).getTime()*/],
      function (View) {
          ...
          switchView: function switchView(inView, outView) {
              ...
              if (outView && !document.getElementById(outView.id)) {
        ...
        function (text) {
          ...
          */
          ...
          showDialog: function showDialog(config, opts) {
              ...
              that.getData(config, function (datas) {
