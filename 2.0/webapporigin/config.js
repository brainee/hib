(function () {
  ...
  require.config({
    ...
    paths: {
      ...
      'cUtilityDate':            app + 'util/c.utility.date',
      // Date对象，对时间提供一些常用方法
      'cUtilityServertime':      app + 'util/c.utility.servertime',
      'cUtilityCrypt':           app + 'util/c.utility.crypt',
      'cUtility':                app + 'util/c.utility',
      'Validate':                app + 'util/c.validate',
      //数据验证

      //消息中心
      'cMessageCenter':          app + 'common/c.message.center',
      //-------------------------------------------------------------
      'cCoreInherit':            app + 'core/c.core.inherit',
      // Class类，框架的基础类体系
      //-------------------------------------------------------------
      // Store
      'cAbstractStore':          app + 'store/c.abstract.store',
      //抽象store
      'cAbstractStorage':        app + 'store/c.abstract.storage',
      //抽象storage
      'cStore':                  app + 'store/c.local.store',
      //提供存取具体数据的Store基础类
      'cStorage':                app + 'store/c.local.storage',
      //提供存取localStorage/sessionStorage的静态方法
      'cSessionStore':           app + 'store/c.session.store',
      //提供存取具体数据的Store基础类
      'cSessionStorage':         app + 'store/c.session.storage',
      //提供存取localStorage/sessionStorage的静态方法
      'memStore':                app + 'store/c.memorystore',
      'CommonStore':             app + 'store/c.common.store',
      //公用的store
      ...
      'cMultipleDate':           app + 'model/c.multiple.data',
      //多重数据对象
      //-----------------------------------------------------------
      // UI组件
      'cUI':                     app + 'ui/c.ui',
      ...
      'cDataSource':             app + 'ui/c.ui.datasource',
      //数据源
      ...
      'cUIBusinessGroupList':    app + 'ui/c.ui.business.group.list',
      //新的citylist，航班动态，可直接使用
      'cUITab':                  app + 'ui/c.ui.tab',
      //标签插件
      ...
      'cPageView':               app + 'page/c.page.view',

      //--------------------------------------------------------------
      // Hybrid
      'cHybridFacade':           app + 'hybrid/c.hybrid.facade',

      'cUIHeader':       app + 'ui/c.ui.header',
      'cBusinessCommon': app + 'business/c.business.common'
