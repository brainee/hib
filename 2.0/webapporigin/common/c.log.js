...
define(['libs'], function() {
  ...
  cLog.checkServerLog = _isExistServerLog(window);
   ...
   * @param response
   */
  cLog.serverLog = function(url, request, response) {
  ...
  cLog.appLog = function(name, param) {
    ...
    var logStr = _parse(appLog);
    localStorage.setItem('APP_LOG', logStr);
  };

  cLog.applog = function(name, param) {
    this.appLog(name, param);
  };
  /**
   ...
   * @param data
   */
  cLog.sendLog = function(type, data, callback) {
    $.post("http://" + this.logServerUrl + '/' + type, data, function(response) {
