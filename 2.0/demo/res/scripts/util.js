define([], function () {
    ...
    */
    ...
    util.getEnvCode = function (cUtility) {
        ...
        } else if (host.match(/^m\.ctrip\.com/i) || host.match(/^(localhost|10\.8|2\.111)/i)) { //生产//堡垒
            return '2';
        } else if (host.match(/^waptest\.ctrip\.com/i) || host.match(/^(localhost|172\.16|127\.0)/i) || host.match(/^(192\.168\.253\.1)/i)) { //测试
            return '0';
        } else { //其他自定义域名
            return '1';
        }
