 ...
 * @requires Calendar
 * @class ChineseHolidayCalendar
 * @description 中国节日日历, 参数会继承于日历插件
 * @example
  ...
  var HolidayCalendar = Calendar.extend({
    ...
    getDateDesc: function(date) {
      ...
      if (options.solarHoliday[holidayKey]) {
        ...
        dateDesc.cls = this.options.holidayCls;
      } else {
        lunarObj = toLunar(date);
        holidayKey = this.generateHolidayKey(lunarObj.month, lunarObj.day);
        //农历节日判断
        if (options.lunarHoliday[holidayKey]) {
          dateDesc.desc = options.lunarHoliday[holidayKey];
          dateDesc.cls = this.options.holidayCls;
        }
      }
